...
** If there are only ranges, the selection will depend on range intersections:
*** If all the ranges intersect, then the highest _existing_ version of the intersection will be selected.
*** If there isn't an intersection between the ranges, the highest _existing_ version will be selected based on the
** If a version declared as `strictly` is lower than that version, selection will fail.
